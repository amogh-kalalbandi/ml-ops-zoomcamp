...
def get_input_files_from_s3():
    ...
    s3_path = variables.get('INPUT_S3_PATH', default='s3://mlflow-artifacts-remote-amogh/training_data/')
...
def move_latest_model_to_registry(experiment_id, mlflow_run_id):
    ...
    for each_run in runs:
        ...
        if each_run.info.run_id == mlflow_run_id:
            ...
            client.set_model_version_tag(model_details.name, model_details.version, "model_type", "xgboost")
            client.set_model_version_tag(model_details.name, model_details.version, "run_type", "latest")
            client.transition_model_version_stage(
